...
class TraceDataset(Dataset):
    """Custom PyTorch Dataset for loading website fingerprint traces."""
    ...
    def __init__(self, data):
        ...
        for entry in data:
            ...
            label = entry["website_index"]
            website = entry["website"]
            
            # Pad or truncate traces to fixed length
            processed_trace = self.process_trace(trace)
            
            self.traces.append(processed_trace)
        ...
        # Get unique website names in order
        self.unique_websites = sorted(list(set(self.website_names)))
    
    def process_trace(self, trace):
        """Process a single trace to fixed length."""
        ...
        return np.array(trace[:INPUT_SIZE])
...
def main():
    """Main function to train and evaluate website fingerprinting models."""
    # 1. Load and prepare dataset
    print("Loading dataset...")
    raw_data = load_dataset(DATASET_PATH)
    dataset = TraceDataset(raw_data)
    
    # Get website names for classification report
    website_names = dataset.unique_websites
    ...
    for model_name, model in models.items():
        ...
        optimizer = optim.Adam(model.parameters(), lr=LEARNING_RATE)
         # Custom naming for merged dataset models
        if model_name == "Basic CNN":
            save_path = os.path.join(MODELS_DIR, "merged_basic_cnn.pth")
        elif model_name == "Complex CNN":
            save_path = os.path.join(MODELS_DIR, "merged_complex_cnn.pth")
        
        # Train the model
        best_accuracy = train(
            model, train_loader, test_loader, 
            criterion, optimizer, EPOCHS, save_path
        )
